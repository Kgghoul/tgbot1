...
# Обработчик для вызова проверки неактивных пользователей
@dp.message_handler(commands=["check_inactive"])
async def cmd_check_inactive_wrapper(message: types.Message):
    """Обертка для вызова проверки неактивных пользователей"""
    await handlers.cmd_check_inactive(message)
