...
async def process_emoji_answer(message: types.Message, state: FSMContext):
    try:
        ...
        logger.debug(f"Текущая загадка: {emoji_game.current_riddle} -> {emoji_game.current_answer}")
        logger.debug(f"Ответ пользователя: {user_answer}, осталось попыток: {attempts}")
        
        if emoji_game.check_answer(user_answer):
            # Сохраняем правильный ответ, так как он может быть сброшен при очистке состояния
            correct_answer = emoji_game.current_answer
            
            # Начисляем баллы за правильный ответ
            success = await db.add_game_activity(chat_id, user_id, "emoji_game", EMOJI_GAME_POINTS)
            
            if success:
                points_text = f"Вы получаете +{EMOJI_GAME_POINTS} баллов активности!"
            else:
                points_text = "Но произошла ошибка при начислении баллов. Попробуйте позже."
                
            await message.answer(
                f"🎉 Правильно, {message.from_user.first_name}! Это действительно '{correct_answer}'.\n{points_text}",
                parse_mode=types.ParseMode.MARKDOWN
            )
            
            logger.info(f"Пользователь {user_id} правильно ответил на загадку")
            await safe_finish_state(state)
        else:
            attempts -= 1
            if attempts > 0:
                await message.answer(
                    f"❌ Неправильно. Осталось попыток: {attempts}",
                    parse_mode=types.ParseMode.MARKDOWN
                )
                await state.update_data(attempts=attempts)
                logger.debug(f"Неправильный ответ, осталось попыток: {attempts}")
            else:
                # Сохраняем правильный ответ, так как он может быть сброшен при очистке состояния
                correct_answer = emoji_game.current_answer
                
                await message.answer(
                    f"😔 Попытки закончились. Правильный ответ: '{correct_answer}'.\n"
                    f"Попробуйте еще раз позже!",
                    parse_mode=types.ParseMode.MARKDOWN
                )
                logger.info(f"Пользователь {user_id} исчерпал все попытки")
                await safe_finish_state(state)
    except Exception as e:
        error_traceback = traceback.format_exc()
        logger.error(f"Ошибка в обработчике ответов emoji_game: {e}\n{error_traceback}")
        await message.answer(f"Произошла ошибка при обработке ответа. Игра остановлена.")
        await safe_finish_state(state)
...
async def process_quiz_answer(message: types.Message, state: FSMContext):
    try:
        user_id = message.from_user.id
        chat_id = message.chat.id
        logger.info(f"Получен ответ от пользователя {user_id} на викторину: {message.text}")
        
        # Проверка, что вопрос был корректно установлен
        if not quiz_game.current_question:
            logger.error(f"Текущий вопрос не найден, перезапускаем игру")
            await message.answer("Произошла ошибка с текущим вопросом. Начните игру заново командой /quiz")
            await safe_finish_state(state)
            return
            
        try:
            user_answer = int(message.text) - 1  # Корректируем, т.к. индексы начинаются с 0
        except ValueError:
            await message.answer("Пожалуйста, введите число для выбора варианта ответа")
            return
            
        # Проверка, что ответ корректный (в пределах диапазона)
        if user_answer < 0 or user_answer >= len(quiz_game.current_question["options"]):
            await message.answer("Пожалуйста, выберите вариант из предложенных (введите число)")
            return
            
        # Сохраняем данные вопроса, т.к. они могут быть сброшены при очистке состояния
        correct_index = quiz_game.current_question["correct"]
        correct_option = quiz_game.current_question["options"][correct_index]
            
        if quiz_game.check_answer(user_answer):
            # Начисляем баллы за правильный ответ
            success = await db.add_game_activity(chat_id, user_id, "quiz", QUIZ_GAME_POINTS)
            
            if success:
                points_text = f"Вы получаете +{QUIZ_GAME_POINTS} балла активности!"
            else:
                points_text = "Но произошла ошибка при начислении баллов. Попробуйте позже."
                
            await message.answer(
                f"🎉 Правильно, {message.from_user.first_name}! {points_text}",
                parse_mode=types.ParseMode.MARKDOWN
            )
            logger.info(f"Пользователь {user_id} правильно ответил на вопрос викторины")
        else:
            await message.answer(
                f"❌ Неверно. Правильный ответ: {correct_option}",
                parse_mode=types.ParseMode.MARKDOWN
            )
            logger.info(f"Пользователь {user_id} неправильно ответил на вопрос викторины")
        ...
        error_traceback = traceback.format_exc()
        logger.error(f"Ошибка в обработчике ответов quiz: {e}\n{error_traceback}")
        await message.answer(f"Произошла ошибка при обработке ответа. Викторина остановлена.")
        await safe_finish_state(state)
